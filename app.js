...
function render(){
  ...
  if(cur.type==='decProperty'||cur.type==='decFinancial'||cur.type==='coDecProperty'||cur.type==='coDecFinancial'){
    ...
    h+='<button class="lang-btn" onclick="toggleLang()">'+t.lang+'</button><div style="margin-bottom:6px"><div style="margin-bottom:8px;padding-right:60px"><span class="sec-label">'+cur.section+'</span></div><div class="progress-bar"><div class="progress-fill" style="width:'+((step+1)/total*100)+'%"></div></div><div style="text-align:right;margin-top:4px;font-size:.72rem;color:#a0926e">'+(step+1)+' '+t.of+' '+total+'</div></div>';
  ...
  if(cur.type==='attestation'){h+='<div style="margin-bottom:6px"><div style="margin-bottom:8px;padding-right:60px"><span class="sec-label">'+cur.section+'</span></div><div class="progress-bar"><div class="progress-fill" style="width:'+((step+1)/total*100)+'%"></div></div><div style="text-align:right;margin-top:4px;font-size:.72rem;color:#a0926e">'+(step+1)+' '+t.of+' '+total+'</div></div><h2 style="font-size:1.15rem;font-weight:700;color:#3a2e1e;margin:16px 0 12px">'+t.attestTitle+'</h2><div class="attest-box"><p class="attest-text">'+t.attestText+'</p></div><div class="check-row" onclick="attested=!attested;render()"><div class="checkbox'+(attested?' checked':'')+'">'+(attested?'\u2713':'')+'</div><span style="font-size:.88rem;color:#4a3f32;line-height:1.4">'+t.attestCheck+'</span></div><button class="primary" '+(attested?'':'disabled')+' onclick="goNext()">'+t.review+' \u2192</button><div class="nav-center"><button class="ghost" onclick="goBack()">'+t.back+'</button></div>';app.innerHTML=h;return;}
  h+='<button class="lang-btn" onclick="toggleLang()">'+t.lang+'</button><div style="margin-bottom:6px"><div style="margin-bottom:8px;padding-right:60px"><span class="sec-label">'+(cur.section||'')+'</span></div><div class="progress-bar"><div class="progress-fill" style="width:'+((step+1)/total*100)+'%"></div></div><div style="text-align:right;margin-top:4px;font-size:.72rem;color:#a0926e">'+(step+1)+' '+t.of+' '+total+'</div></div><div class="q-area"><h2 class="q-title">'+cur.q+'</h2>';
...
window.validateMultiField = function(inp) {
  var format = inp.getAttribute('data-format');
  if (!format) return;
  var v = inp.value;
  var isValid = !v || (format === 'email' ? validEmail(v) : (format === 'phone' ? validPhone(v) : true));
  inp.style.borderColor = (v && !isValid) ? '#c83232' : '';
  var errDiv = inp.parentNode.querySelector('.field-err');
  if (v && !isValid) {
    if (!errDiv) { errDiv = document.createElement('div'); errDiv.className = 'field-err'; errDiv.style.cssText = 'color:#c83232;font-size:.72rem;margin-top:4px;padding-left:4px'; inp.parentNode.appendChild(errDiv); }
    errDiv.textContent = format === 'email' ? (lang==='en'?'Enter a valid email address':'请输入有效的电子邮箱') : (lang==='en'?'Enter a 10-digit phone number':'请输入有效的10位电话号码');
  } else if (errDiv) { errDiv.remove(); }
};
render();
